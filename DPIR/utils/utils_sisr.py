...
def data_solution(x, FB, FBC, F2B, FBFy, alpha, sf):
    ...
    # multiplica numerador por F(k)
    x1 = cmul(FB, FR)
    # en caso de deblurring FBR=x1
    FBR = torch.mean(splits(x1, sf), dim=-1, keepdim=False)
    # en caso de deblurring invW=F2B
    invW = torch.mean(splits(F2B, sf), dim=-1, keepdim=False)
    # para mi invWBR = FB * F(x)
    invWBR = cdiv(FBR, csum(invW, alpha))
    # para mi FCBinvWBR = F2B * F(x)
    FCBinvWBR = cmul(FBC, invWBR.repeat(1, 1, sf, sf, 1))
    FX = (FR-FCBinvWBR)/alpha.unsqueeze(-1)
    Xest = torch.irfft(FX, 2, onesided=False)
    return Xest

def data_solution_non_uniform(x, FB, FBC, F2B, STy, alpha, sf):
...
def data_solution_GC(x, FB, FBC, F2B, FBFy, alpha, sf):
    '''
    Args:
        y: NxCxHxW, LR input
        k: NxCxhxw
    ...
    Returns:
    '''
    aFz = torch.rfft(alpha*x, 2, onesided=False)
    X = cdiv(FBFy + aFz, csum(F2B, alpha))
    x = irfft(X)
    return x

# def data_solution_GC(y, z, k, alpha):
#     '''
#     Args:
#         y: NxCxHxW, LR input
#         k: NxCxhxw
...
#     W, H = y.shape[-2:]
#     K = p2o(k, (W, H))
#     Kc = cconj(K, inplace=False)
#     Fy = torch.rfft(y, 2, onesided=False)
#     aFz = torch.rfft(alpha*z, 2, onesided=False)
#     KcFy = cmul(Kc,Fy)
#     KcK = cmul(Kc, K)
#     X = cdiv(KcFy + aFz, KcK + alpha)
#     x = irfft(X)
#     return x

def pre_calculate_non_uniform(x, k, sf):
    '''
    Args:
        x: NxCxHxW, LR input
        k: NxCxhxw
